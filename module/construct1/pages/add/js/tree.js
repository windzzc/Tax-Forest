...
function draw_tree(tree, seed, svgobj, multiple){
	...
	if(multiple>=0.05){
		...
		.append('textPath')
			.style('font-size', function (d) {return (d.type == 'twig')? ('13px'):('17px')})
...
$.ajax({
	...
	success: function(response){
		...
		var tree4 = buildTree(dataset, seed4, temp_scale);
	    draw_tree(tree4, seed4, svg, temp_scale); 
