...
/*********************************************************************************************************************/
var svg = document.getElementById('facet-tree-canvas');
...
app.controller("facetTreeController", function ($scope, $http) {
    ...
    $http.get(ip + '/topic/getTopicsByDomainName?domainName=' + getCookie("NowClass")).success(function (data) {
        ...
        var topic = response[0].topicName;

        setCookie("topic", topic, "d900");
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(chil, function (i, val) {
                    ...
                    $(".fen").append("<li class='fe animated bounceInRight' onclick='assemble(this)'>" + val.facetName + "</li>");
                    console.log(val.facetName);
                });
                console.log(data);
                $(svg).empty();
                facetTree.drawTree(svg, data, () => {});
                // DisplayBranch(data);
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
